...
import { INDIA_TOUR_KEYS } from '../constants/enquiryData';
import './Hero.css';
...
const Hero = ({ onOpenEnquiry, onGetEstimate }) => {
  ...
  // The search fields are divs (they contain nested real <button>s, so they
  // can't be <button> themselves) — give them button semantics + keyboard.
      ...
      <div className="container hero-inner">
        <div className="hero-content">
          ...
          <div className="hero-search-bar" ref={barRef}>
            ...
            {/* Destination */}
            <div
              className="hero-search-field hero-search-field--btn"
              role="button"
              tabIndex={0}
              aria-haspopup="listbox"
              aria-expanded={openField === 'destination'}
              onClick={() => toggle('destination')}
              onKeyDown={fieldKeyDown('destination')}
            >
              ...
              <span className={destination ? 'hero-field-value' : ''}>{destination || t('hero.destination', { defaultValue: 'Destination' })}</span>
              ...
              {openField === 'destination' && (
                <ul className="hero-field-dropdown">
                  {INDIA_TOUR_KEYS.map(key => {
                    const title = t(key);
                    return (
                      <li key={key} className={destination === title ? 'selected' : ''} onMouseDown={(e) => { e.preventDefault(); setDestination(title); setOpenField(null); }}>
                        {title}
                      </li>
                    );
                  })}
            ...
            <div
              ...
              onKeyDown={fieldKeyDown('travellers')}
            >
              ...
              <span className="hero-field-value">{travellers} {travellers === 1 ? t('hero.traveller', { defaultValue: 'Traveller' }) : t('hero.travellers', { defaultValue: 'Travellers' })}</span>
              <svg className="hero-search-chevron" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true"><path d="m6 9 6 6 6-6"/></svg>
              {openField === 'travellers' && (
                <div className="hero-field-dropdown hero-field-dropdown--travellers" onMouseDown={(e) => e.preventDefault()}>
                  <span className="hero-traveller-label">{t('contact.form.travelers', { defaultValue: 'Number of Travelers' })}</span>
                  ...
                  <button type="button" className="hero-done-btn" onClick={(e) => { e.stopPropagation(); setOpenField(null); }}>{t('hero.done', { defaultValue: 'Done' })}</button>
                </div>
              )}
            </div>
            ...
            <div
              ...
              tabIndex={0}
              aria-haspopup="listbox"
              aria-expanded={openField === 'days'}
              onClick={() => toggle('days')}
              onKeyDown={fieldKeyDown('days')}
            >
              ...
              <span className={days ? 'hero-field-value' : ''}>{days || t('hero.selectDays', { defaultValue: 'Select Days' })}</span>
              <svg className="hero-search-chevron" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true"><path d="m6 9 6 6 6-6"/></svg>
              {openField === 'days' && (
                <ul className="hero-field-dropdown">
                  {DAYS_OPTIONS.map(d => (
            ...
            <button className="hero-search-btn" onClick={handleEstimate}>
              {t('hero.getEstimate', { defaultValue: 'Get Estimate' })}
            </button>
          </div>
        </div>
      </div>
